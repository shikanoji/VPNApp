    ...
    var body: some View {
        HStack(spacing: 16) {
            if #available(iOS 15.0, *) {
                AsyncImage(url: URL(string: $node.ensign.wrappedValue)) { phase in
...
//                            .frame(width: imageSize, height: imageSize)
                ...
                ImageView(withURL: $node.ensign.wrappedValue)
                    .clipShape(Circle())
            }
...
struct NodeCellView_Previews: PreviewProvider {
    static var previews: some View {
        NodeCellView(node: Node.simple1)
